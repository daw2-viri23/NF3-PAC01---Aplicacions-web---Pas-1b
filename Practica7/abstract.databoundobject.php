...
abstract class DataBoundObject {
   ...
   public function Load() {
      if (isset($this->ID)) {
        ...
        $strQuery .= " FROM " . $this->strTableName . " WHERE \"language_id\" = :eid";
        $objStatement = $this->objPDO->prepare($strQuery);
        $objStatement->bindParam(':eid', $this->ID, PDO::PARAM_INT);
        $objStatement->execute();
        $log = Logger::getInstance();
         $log->logMessage("Select2 de la clase Language ");
   ...
   public function Save() {
      if (isset($this->ID)) {
         ...
         $strQuery .= ' WHERE "language_id" = :eid';
         ...
         $objStatement->execute();
         $log = Logger::getInstance();
         $log->logMessage("Update2 de la clase Language ");
      } else {
         $strValueList = "";
         ...
         echo $this->ID; 
         $log = Logger::getInstance();
         $log->logMessage("Insert2 de la clase Language ");
      }
   ...
   public function __destruct() {
      if (isset($this->ID)) {   
         if ($this->blForDeletion == true) {
            $strQuery = 'DELETE FROM "' . $this->strTableName . '" WHERE "language_id" = :eid';
            $objStatement = $this->objPDO->prepare($strQuery);
            $objStatement->bindValue(':eid', $this->ID, PDO::PARAM_INT);   
            
            $objStatement->execute();
            $log = Logger::getInstance();
            $log->logMessage("Delete2 la clase Language ");
         };
      }
   }
